...
export function ExamProgress({
  ...
  return <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
      ...
      return <button key={idx} onClick={() => onJumpToQuestion(idx)} className={`
            ...
            `}>
            {idx + 1}
            {isFlagged && <span className="absolute -top-1 -right-1 h-2.5 w-2.5 bg-amber-400 rounded-full border border-white" />}
          </button>;
    })}
    </div>;
}