...
export function QuestionCard({
  ...
  return <Card className="w-full max-w-3xl mx-auto">
      ...
      <div className="space-y-3 mb-8">
        {question.choices.map((choice, index) => <button key={index} onClick={() => onSelectAnswer(index)} className={`
      ...
      <div className="flex justify-end">
        <Button onClick={onSubmit} disabled={selectedAnswerIndex === null || isSubmitting} size="lg" className="w-full sm:w-auto px-8">
          Submit Answer
        </Button>
      </div>
    </Card>;
