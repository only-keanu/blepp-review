      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card>
          <div className="space-y-2">
            {topics.map(topic => <div key={topic.id} className="flex items-center justify-between p-4 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                <span className="font-medium text-slate-900">{topic.name}</span>
                <button onClick={() => toggleWeak(topic.id)} className={`
